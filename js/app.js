...
  const cfg = window\.CLIPCLASH\_CONFIG || {};

  const configured = Boolean(

    cfg.SUPABASE\_URL &&

    cfg.SUPABASE\_PUBLISHABLE\_KEY &&

    !cfg.SUPABASE\_URL.includes('YOUR\_') &&

    !cfg.SUPABASE\_PUBLISHABLE\_KEY.includes('YOUR\_')
...
    ? window\.supabase.createClient(cfg.SUPABASE\_URL, cfg.SUPABASE\_PUBLISHABLE\_KEY)
...
      const label = state.profile?.display\_name || state.profile?.username || state.user.email || 'Player';
...
      {id:'demo1',title:'Last-second clutch 😭',game:'Fortnite',created\_at:new Date(Date.now()-8\*60000),profiles:{username:'nova',display\_name:'Nova'},likes:[1,2,3,4,5],clip\_url:'https\://www\.youtube.com/'},

      {id:'demo2',title:'The cleanest escape I have ever hit',game:'GTA V',created\_at:new Date(Date.now()-21\*60000),profiles:{username:'driftkid',display\_name:'DriftKid'},likes:[1,2,3],clip\_url:'https\://www\.youtube.com/'},

      {id:'demo3',title:'This should NOT have worked 💀',game:'Roblox',created\_at:new Date(Date.now()-43\*60000),profiles:{username:'pixel',display\_name:'Pixel'},likes:[1,2],clip\_url:'https\://www\.youtube.com/'}
...
      .select('id,user\_id,title,game,clip\_url,created\_at,profiles(username,display\_name),likes(user\_id)')
...
      ? query.order('created\_at', { ascending: false })
...
        state.clips.sort((a,b) => (b.likes?.length || 0) - (a.likes?.length || 0) || new Date(b.created\_at)-new Date(a.created\_at));
...
      const liked = state.user && likes.some(l => (l.user\_id || l) === state.user.id);
...
        \<a class="clip-cover" data-game="${escapeHTML(clip.game)}" href="${escapeHTML(clip.clip\_url)}" target="\_blank" rel="noopener noreferrer">
...
              \<div class="creator-avatar">${escapeHTML(initials(creator.display\_name || creator.username || 'P'))}\</div>

              \<div class="creator-meta">\<strong>@${escapeHTML(creator.username || 'player')}\</strong>\<span>${relativeTime(clip.created\_at)}\</span>\</div>
...
    const liked = (clip.likes || []).some(l => l.user\_id === state.user.id);



    if (liked) {

      const { error } = await supabase.from('likes').delete().eq('clip\_id', clipId).eq('user\_id', state.user.id);

      if (error) return toast(error.message);

      clip.likes = clip.likes.filter(l => l.user\_id !== state.user.id);

    } else {

      const { error } = await supabase.from('likes').insert({ clip\_id: clipId, user\_id: state.user.id });

      if (error) return toast(error.message);

      clip.likes.push({ user\_id: state.user.id });
...
    if (!/^[A-Za-z0-9\_]{3,20}$/.test(username)) return setMessage(els.authMessage, 'Username: 3–20 letters, numbers or underscores only.');
...
      options: { emailRedirectTo: redirect, data: { username, display\_name: displayName } }
...
    const payload = { user\_id: state.user.id, title: $('clipName').value.trim(), game: $('clipGame').value, clip\_url: $('clipUrl').value.trim() };
...
    $('profileName').textContent = p.display\_name || 'Player';

    $('profileHandle').textContent = \`@${p.username || 'player'}\`;

    $('profileAvatar').textContent = initials(p.display\_name || p.username || 'P');

    $('profileDisplayName').value = p.display\_name || '';
...
        supabase.from('clips').select('\*', { count:'exact', head:true }).eq('user\_id', state.user.id),

        supabase.from('clips').select('id,likes(user\_id)').eq('user\_id', state.user.id)
...
    const { error } = await supabase.from('profiles').update({ display\_name:$('profileDisplayName').value.trim(), bio:$('profileBio').value.trim() }).eq('id', state.user.id);
